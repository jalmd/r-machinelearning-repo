...
model <- svm(x, y,  probability = TRUE, cross = 10, kernel = kernel) 
modelB <- svm(xB, yB, probability = TRUE, cross = 10, kernel= kernel)
print(model)
summary(model)
print(modelB)
summary(modelB)
...
precision_list[[counter_fmeasure]] <- precisions
recall_list[[counter_fmeasure]] <- recalls
f_measure_list[[counter_fmeasure]] <- f_measures
